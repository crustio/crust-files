...
function Index(props: Props) {
  ...
  const { alert, loading, api } = useApp();
  ...
  const [periodTime, days] = useMemo(() => {
    if (!hasDeposit || !bestNumber) return "--:--:--";
    const timeSecends = deposit!.deposit.expire_block_number > bestNumber ? (deposit!.deposit.expire_block_number - bestNumber) * 6 : 0;
    const fmtTime = format((timeSecends > 0 ? cTime + timeSecends : deposit!.deposit.expire_timestamp) * 1000, "yyyy-MM-dd");
    const fmtDays = Math.round(timeSecends / 60 / 60 / 24);
    return [fmtTime, fmtDays];
  }, [hasDeposit, deposit, cTime, bestNumber]);
  useEffect(() => {
    if (!config) return;
  ...
  const disabledClaim = !uClaim.ready || onGoingClaim || (hasDeposit && (bestNumber ?? 0) > 0 && deposit!.deposit.expire_block_number > (bestNumber ?? 0));
  const _onClickDownCrustWallet = () => window.open(CrustWalletDownUrl, "_blank");
  const _onClickDeposit = () => {
    !disabledDeposit && uDeposit.start();
  };
  const _onClickClaim = () => {
    !disabledClaim && uClaim.start();
  };
  ...
  return (
    <PageUserSideLayout path={"/user"} className={className}>
      <div className="user-Info">
        <div className="title">{isPremiumUser ? "You are Premium User" : "You are Trial User"}</div>
        <div className="sub-text">
          Web3 Identity Logged-in: <span>{user.account}</span>
        </div>
      </div>
        ...
        <div className="permium-trial">
          <div className="mtable premium">
            ...
            <div className="t-item">
              <span>Maximum Upload Size of</span>
              <span className="primary">1GB</span>
            </div>
            <div className="t-item two">
              <span style={{ marginRight: 6 }}>
                <span className="primary">Renew</span> On-chain Storage Order &
              </span>
              <span>
                Place <span className="primary">{`"Permanent Storage Order"`}</span>
              </span>
            </div>
            ...
            <div className="t-item">
              <span>Priority access to</span>
              <span className="primary">future airdrops</span>
          ...
          <div className="mtable trial">
            ...
            <div className="t-item">
              <span>Maximum Upload Size of </span>
              <span className="primary">40MB</span>
            </div>
            <div className="t-item">
              <span>Storage Order Expires in</span>
              <span className="primary">6 Months</span>
            </div>
            <div className="t-item">
              <span className="primary">Limited</span> <span>Space</span>
            </div>
            <div className="t-item">
              <span className="primary">Cannot</span>
              <span style={{ marginRight: 6 }}>Claim Rewards</span> <span>{`from "Share-and-Earn"`}</span>
            </div>
            <div className="t-item">
              <span className="primary">Limited</span> <span style={{ marginRight: 6 }}>access to future airdrops</span> <span>or other exclusive benefits</span>
            </div>
          </div>
        </div>
      </div>
          ...
          {showPremiumLoading ? (
            ...
            <>
              <div className="text font-sans-regular">
                {showBasePrice ? (
                  <>
                    Deposit <span className={classNames("origin", "isBase")}>{baseGuaranteeAmount} CRU</span>
                    <span className="reffer">(now {guaranteeAmount} CRU for Discount!)</span> to become a Premium User. <br />
                    Deposit <span className={classNames("origin", "isBase")}>{baseGuaranteeDiscountWithReferer} CRU</span>
                    <span className="reffer">(now {guaranteeDiscountWithReferer} CRU for Discount!)</span> if you have an invitation code (the Nickname of your inviter).
                    <br />
                  </>
                ) : (
                  <>
                    Deposit <span className={classNames("origin")}>{guaranteeAmount} CRU</span> to become a Premium User. <br />
                    Deposit <span className={classNames("origin")}>{guaranteeDiscountWithReferer} CRU</span> if you have an invitation code (the Nickname of your inviter).
                    <br />
                  </>
                )}
              ...
              <div className={"btns mbtns"}>
                <input className="input-Nickname" spellCheck="false" placeholder="Enter inviter’s Nickname（Leave blank if you have no inviter.）" onChange={_onChangeNickname} />
                ...
                <a href="/docs/CrustFiles_Users" target="_blank">
                  How to deposit?
                </a>
              </div>
            </>
      ...
      {showClaim && (
        <MCard>
          ...
          <div className="text font-sans-regular">
            You can redeem your deposit after {days} days (est. on{" "}
            <span className="font-sans-semibold" style={{ color: "#333333" }}>
              {periodTime}
            </span>
      ...
      {isPremiumUser && !loadNft && (
        <MCard>
          ...
          <div className="text font-sans-regular">Collect your Achievement Badge here.</div>
          {hasBadge ? (
            <div style={{ marginTop: 20, display: "flex", height: 100 }}>
              <BadgeIcon2 />
              <ColFlex style={{ marginLeft: 18, flex: 1, justifyContent: "space-between" }}>
                ...
                <div>
                  <a className="mlink-btn" target="_blank" href={`${badgeEnpoint}/ipfs/${badgeCID}?filename=${badgeFileName}`} rel="noreferrer">
                    View full image
                  </a>
                  <a className="mlink-btn" target="_blank" href={`https://ipfs-scan.io/?cid=${badgeCID}`} rel="noreferrer">
                    Check in IPFS Scan
                  </a>
                </div>
              </ColFlex>
            ...
            <>
              <div style={{ width: 0, height: 0, overflow: "hidden" }}>
                <Badge name={user.nickName} ref={badgeRef as any} />
              </div>
