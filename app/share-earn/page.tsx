...
import type { BlockNumber } from "@polkadot/types/interfaces/types";
...
const YouComment = styled.div`
  ...
  background-size: 4.71rem 100%, calc(100% - 6.14rem + 0.29rem) 100%, 1.43rem 100%;
...
const YourFriendComment = styled.div`
  ...
  background-size: 1.64rem 100%, calc(100% - 6.36rem + 0.29rem) 100%, 4.71rem 100%;
...
const PixelProgress = styled(PixelBoard).attrs<{ progress?: number; board_size?: number }>(pixelProgressAttrs)`
  height: 14px;
  width: 104px;
...
const M_PixelBtn = styled(PixelBtn).attrs<{ height?: number }>((p) => ({ height: p.height || 40 }))`
  margin-top: 1.43rem;
  .btn_content {
    white-space: nowrap;
    padding: unset;
...
function Index(props: Props) {
  ...
  const isGrandDrawWiner = useMemo(() => {
    return !!(user.account && granDraw && granDraw.members && granDraw.members.find((item) => isSameCrustAddress(item.address, user.account)));
  ...
  useEffect(() => {
    const n = bestNumFinlized ? bestNumFinlized.toNumber() : 0;
    setBNF((last) => {
      if (!last) return n;
      return last;
    });
  ...
  const myTicket = useMemo(() => {
    if (depositDto && granDraw && depositDto.extrinsic_hash) return depositDto.extrinsic_hash.substring(depositDto.extrinsic_hash.length - granDraw.grandDraw.matchCount);
    return "--";
  }, [granDraw, depositDto]);
  ...
  return (
    <PageUserSideLayout path="/share-earn" className={className}>
      <div className="share-and-earn">
        ...
        <PixelBoard className="top_card">
          <div className="btns">
            <PixelBtn1 className={classNames("style_left", { dark: !isMe })} onClick={() => setIsMe(true)}>
              Me
            </PixelBtn1>
            <PixelBtn1 className={classNames("style_right", { dark: isMe })} onClick={() => setIsMe(false)}>
              Network
            </PixelBtn1>
          </div>
          {isMe ? (
            <>
              ...
              <div className="total_reward">
                <span>{getFormatValue(rewards, "total.total")}</span> CRU
              </div>
              <ClaimRewards>
                ...
                {isPremiumUser ? (
                  ...
                  <div style={{ marginTop: 8 }}>
                    <ColorSpan className="btn" onClick={() => r.push("/user")}>
                      Get Premium
                    </ColorSpan>{" "}
                    User to claim your rewards.
                  </div>
                )}
              </ClaimRewards>
            </>
            ...
            <div className="networks">
              <div>
                Total User: <span>{getFormatValue(networkState, "totalUser")}</span>
              </div>
              <div>
                Premium User: <span>{getFormatValue(networkState, "premiumUser")}</span>
              </div>
              <div>
                Deposit Pool: <span>{getFormatValue(networkState, "depositPool")} CRU</span>
              </div>
              <div>
                Rewards Distributed: <span>{getFormatValue(networkState, "rewardsDistributed")} CRU</span>
              </div>
      ...
      <PixelBoard className="earn_item">
        ...
        <div className="left">
          <InviteBonusTitle>
            Invite friends to Crust
            <br />
            Files and earn <span>CRU</span>.<br />
            <MLink target={"_blank"} href={locationUrl("/invite_bonus_guide")}>
              How to invite?
            </MLink>
          </InviteBonusTitle>
        </div>
        <You />
        <YouComment>
          Get <span>{youGet} CRU</span> Reward
        </YouComment>
        <YourFriend />
        <YourFriendComment>
          Get <span>{youFriendGet} CRU</span> Discount on
          <br />
          Premium User Deposit
        </YourFriendComment>
        <DetailedRules target={"_blank"} href={locationUrl("/docs/CrustFiles_ShareandEarn/#invite_bonus")}>
          Detailed Rules
        </DetailedRules>
      </PixelBoard>
      ...
      <PixelBoard className="earn_item" style={{ display: "none" }}>
        ...
        the countdown to {luckyDutation}min and add {depositRewardAmount} CRU into the pool.<br /> */}
          The first Newbie Prize of 15,000 CRU has been claimed by{" "}
          <a target={"_blank"} href={`${locationUrl("/rewards_history/")}?account=${user.account}`} rel="noreferrer">
            this lucky user
          </a>
          .<br />
          More exciting User Rewards Programs are coming soon...
          <div className="footer">{/* Best chance: <span>{shortStr(getStrValue(luckyNebie, 'memberAddress'), 12)}</span> */}</div>
        </div>
        <DetailedRules target={"_blank"} href={locationUrl("/docs/CrustFiles_ShareandEarn/#lucky_newbie")}>
          Detailed Rules
        </DetailedRules>
      </PixelBoard>
      ...
      {showGrandDraw && (
        <PixelBoard className="earn_item">
          ...
          <div className="left">
            <CoinCru>
              <Coin /> {getFormatValue(granDraw, "grandDraw.totalAmount")} CRU
            </CoinCru>
            <Trophy />
            {GrandStat0 && (
              <GrandDrawProgress>
                <PixelProgress progress={grandProgress} /> <span>{_.clamp(getStrValue(granDraw, "depositCount"), 0, getStrValue(granDraw, "grandDraw.memberSize"))}</span>/
                {getStrValue(granDraw, "grandDraw.memberSize")}
              </GrandDrawProgress>
            )}
          ...
          <div className="right">
            <div className="title">
              ...
              {isPremiumUser ? (
                ...
                <span style={{ color: "#333333", fontSize: 20, verticalAlign: "middle" }}>
                  - Get{" "}
                  <span style={{ color: "#FF8D00", cursor: "pointer" }} onClick={_clickGetPremium}>
                    Premium User
                  </span>{" "}
                  to join the game.
                </span>
            ...
            {GrandStat1 && (
              <div>
                Sign up for the draw before Block <span>#{getFormatValue(granDraw, "grandDraw.expireBlock")}</span>
                <br />
            ...
            {GrandStat2 && (
              <div>
                Block Hash of #{getFormatValue(granDraw, "grandDraw.expireBlock")}: <span>{hash}</span>
                <br />
                Winning Condition: <span>The last {getStrValue(granDraw, "grandDraw.matchCount")} digits match</span>
                <br />
                My Status: <span>{isGrandDrawWiner ? "You are the Winner!" : !isMatched ? "Not Matched" : "Not Signed Up"}</span>
              </div>
            )}
          </div>
          <DetailedRules target={"_blank"} href={locationUrl("/docs/CrustFiles_ShareandEarn/#grand_draw")}>
            Detailed Rules
          </DetailedRules>
        </PixelBoard>
      )}
      ...
      <PixelBoard className="earn_item" style={{ display: "none" }}>
        <Rewards>
          ...
          <div className="sub">
            Total Rewards: <span>{getFormatValue(rewards, "total.total")}</span> CRU (<span>{getFormatValue(rewards, "total.claimed")}</span> CRU Claimed)
            <br />
            Invite Bonus: <span>{getFormatValue(rewards, "reward.total")}</span> CRU (<span>{getFormatValue(rewards, "reward.claimed")}</span> CRU Claimed)
            <br />
            Lucky Newbie: <span>{getFormatValue(rewards, "depositReward.total")}</span> CRU (<span>{getFormatValue(rewards, "depositReward.claimed")}</span> CRU Claimed)
            <br />
          ...
          {isCrust && (
            <MLink style={{ marginTop: "1rem" }} target={"_blank"} href={`${locationUrl("/rewards_history/")}?account=${user.account}`}>
              Check more detailed information & historical records about rewards
            </MLink>
          )}
        </Rewards>

        <DetailedRules target={"_blank"} href={locationUrl("/docs/CrustFiles_ShareandEarn/#claim_rewards")}>
          Learn More
        </DetailedRules>
      </PixelBoard>
    </PageUserSideLayout>
