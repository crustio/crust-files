...
function _share(props: BaseProps) {
    ...
    const queryCid = query.get("cid")
    const queryOptions = query.get("options")
    ...
    useEffect(() => {
        if (queryCid) {
            const hexStr = queryOptions as string
            const opt = JSON.parse(hexStr) as ShareOptions
            setUiData({ options: opt, cid: queryCid as string })
        }
        if (queryCode) {
            loading.show()
            getShortInfo(queryCode as string)
                .then(v => setUiData({ ...v }))
                .catch(e => alert.error(getErrorMsg(e)))
                .then(loading.hide)
        }
    }, [queryCid, queryCode, queryOptions])
    useEffect(() => {
        if (cid && options) {
            report({
                type: 3,
