...
export const getErrorMsg = (error: any, def = "Network Error"): string => {
  ...
  if (msgLow.includes("denied") || msgLow.includes("rejected") || msgLow.includes("declined")) return "You declined the action in your wallet.";
  if (msg.includes("transaction failed")) return "Transaction failed";
  return msg;
};

export function genPromise<T>() {
  let reslove = (res: T | PromiseLike<T>) => {};
