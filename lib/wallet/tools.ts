...
export const getPerfix = (user: LoginUser): string => {
  if (user.wallet.startsWith("metamask") || user.wallet === "metax" || user.wallet === "wallet-connect" || user.wallet === "baseminikit") {
    return "eth";
  }
  if (user.wallet === "flow") {
    return "flow";
...
export async function updateAuth(u: LoginUser, w: BaseWallet = WALLETMAP[u.wallet]) {
  ...
  const base64Auth = signature.length > 1000 ? DefaultAuth : window.btoa(authdata);
