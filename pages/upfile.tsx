...
function _upfile(props: BaseProps) {
    ...
    const [sf, setSF] = useState<SaveFile>(null)
    const copy = useClipboard()
    const { query, push } = useRouter()
    const endpoints = useMemo(() => {
        const urls = typeof query.endpoints == 'string' ? [query.endpoints] : query.endpoints;
        return urls?.map(url => ({
            value: url,
            text: url,
        }) as AuthIpfsEndpoint)
                ...
                {sf && <div className='cid-info'>{shortStr(sf.Hash)} <span onClick={() => copy(sf.Hash)} style={{ cursor: 'pointer' }} className="cru-fo cru-fo-copy" /></div>}
                {
                    wInputFile.file && <UpModal
                        ...
                        onSuccess={(file) => {
                            ...
                            if (query.redirect) {
                                const redirect = new URL(query.redirect as any)
