...
function _GetNickname(props: BaseProps) {
  ...
  const [_loading, setLoading] = useToggle();
  useEffect(() => {
  ...
  const doCheckNickName = useMemo(() => {
    ...
    return _.debounce((nickName: string) => {
      ...
      checkNickName(nickName, { cancelToken: task.token }).then((valid) => {
  ...
  const showGetNickname = _.isEmpty(member) && account && wallet === "crust" && !wUser.nickName && !_loading;
  ...
  return (
    <div className={className}>
      ...
      <span className="sub-info">
        This Nickname is unique, linked to your Crust Account, and cannot be changed afterwards. Get a nice one!
      </span>
      <div className="input-panel">
        ...
        <span className={classNames("btn-continue", { enabled: nickStat === 1 })} onClick={_onClickContinue}>
          Continue
        </span>
      </div>
