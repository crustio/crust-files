...
function readFile(entry: FileSystemFileEntry, path: string): Promise<File> {
  return new Promise((resolve) => {
    entry.file((file) => {
      (file as any)._webkitRelativePath = `${path}/${file.name}`;
      resolve(file);
    });
  });
}

function readEntries(reader: FileSystemDirectoryReader, path: string): Promise<File[]> {
  return new Promise((resolve) => {
    let data: File[] = [];
    reader.readEntries(async (entries: FileSystemEntry[]) => {
      ...
      if (length) {
        for (const itemEntry of entries) {
          if (itemEntry.isFile) {
            data.push(await readFile(itemEntry as FileSystemFileEntry, path));
          } else if (itemEntry.isDirectory) {
            data = data.concat(await readEntries((itemEntry as FileSystemDirectoryEntry).createReader(), `${path}/${itemEntry.name}`));
          }
        }
...
async function getFileInfo(data: DataTransfer): Promise<FileInfo | null> {
  ...
  const readers: { reader: FileSystemDirectoryReader; path: string }[] = [];
  for (let i = 0; i < files.length; i++) {
    ...
    if (entry && entry.isDirectory) {
      ...
      readers.push({ reader: (entry as FileSystemDirectoryEntry).createReader(), path: entry.name });
    } else if (entry) {
      if (isOne) {
