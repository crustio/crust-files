...
function MDropdown(props: Props) {
  ...
  return (
    <div className={classNames(className, "mdropdown")} onClick={() => toggleVisible()} ref={ref}>
      {label && (
        <div className="label font-sans-semibold">
          {label}
          {help && <Popup position={"top center"} trigger={<span className="icon cru-fo-help-circle" />} content={help} />}
        </div>
      )}
  ...
  styled(MDropdown)`
    &.mdropdown {
      ...
      .options {
        ...
        max-height: ${(p) => p.optionsMaxHeight || "300px"};
        overflow-y: auto;
        position: absolute;
        top: calc(100% + 1px);
        left: 0;
